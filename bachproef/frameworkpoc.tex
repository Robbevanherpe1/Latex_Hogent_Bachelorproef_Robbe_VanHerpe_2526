...
Het Competentie framework PL1 / CICS developer op z/OS level 1 vertrekt vanuit volgende punten.
...
Een derde criteria is een sterke focus op de praktijk.
...
\subsubsection{De link met CICS}
\label{sec:link-met-cics}

Een vierde criteria is de integratie met CICS.
...
\subsubsection{Hedendaagse IT landschap}
\label{sec:Hedendaagse-IT-landschap}

Een laatste criterium is de connectie met het hedendaagse IT landschap.
Legacy systemen zoals IBM mainframes opereren binnen een groter geheel,
om te begrijpen waarom deze talen vandaag de dag nog steeds cruciaal zijn voor grote ondernemingen moet er in het framework ook aandacht zijn voor modernisering,
en de manier waarop PL/I en CICS toepassingen integreren met moderne architecturen.
...
\begin{itemize}
    ...
    \item \textbf{CICS Transactieverwerking (Competentie 5-7):} Richt zich op CICS concepten en de integratie hiervan in PL/I programma's. Dit is onmisbaar voor het ontwikkelen en begrijpen van online transactiesystemen op de mainframe.
    ...
    \item \textbf{Bedrijfscontext en Onderhoud (Competentie 11-13):} Focust op codeanalyse, de historische context en modernisering. Dit is cruciaal om de blijvende waarde van legacyvsystemen te begrijpen en ze veilig te kunnen onderhouden binnen moderne IT architecturen.
\end{itemize}
...
Bewust is gekozen voor een scherpe afbakening op een basis niveau of junior profiel, dit waarborgt een haalbare leercurve.
De overgang naar een volgende niveau aangeduid als Level 2 of medior profiel markeert de stap van concentuele kennis naar praktishe complexe applicatie ontwikkeling.
...
\subsubsection{Geavanceerde PL1}
...
\subsubsection{Geavanceerde tooling en diagnostiek}
...
\subsubsection{Performantie optimalisatie en efficiëntie}
...
\subsubsection{Security architectuur}
...
De PoC is gerealiseerd als een laagdrempelige leersite die de student begeleid in het verwerven van de gedefinieerde competenties.

Door de chronologie van het framework te respecteren en eerst universele logica, dan de taal, gevolgd door debugging en context wordt het bregen van te moeilijke leerstof vermeden.
Dit verlaagt de drempel aanzienlijk voor IT'ers die nog nooit een mainframe in contact zijn gekomen.
...
\subsection{Bepalen van technologieën}%
...
Het doel van de leersite was een toegankelijke leersite rond PL/I en CICS met theorie, oefeningen en quizzen die vlot kan uitgebreid worden zonder extra hostingkost.
...
Zonder de historische en conceptuele kennis zouden PL/I en CICS kunnen overkomen als niet relevante en eerder verouderde technologieën.
Net die systemen die voor veel bedrijven nog steeds een zeer cruciale rol spelen binnen de financiele sector.
...
Deze module sluit direct aan op competentie 12.0 (Historische en enterprise-context) en competentie 13.0 (Modernisering en integratie) van het competentie framework. 
...
De module \textbf{Environment Setup and ISPF Fundamentals} legt de praktishe basis om te leren werken met ISPF en zowe Xplorer.
Het leren werken met deze code editors behandeld de competentie 2.0 (IBM Z basiskennis) en stelt de student in staat om zij ontwikkelingsomgeving te beheersen.
...
Legacy technologieën zoals ISPF hebben een sterke leercurve voor nieuwe developers, daarom word er ook gefocused in dit thema op alternatieven zoals Zowe Xplorer.
Studenten zijn code editor zoals vscode gewoon en kunnen hierdoor efficienter werken met deze tools.
Daarom is deze module gefocussed op beide manieren om de drempel zo laag mogelijk te maken voor nieuwe ontwikkelaars.
Deze module zou als eindresultaat de student een goede basis moeten geven om zich te navigeren in hun ontwikkelingsomgeving waardoor ze zich kunnen focussen op het leren van PL1 en CICS.
...
Het behandeld enkele competenties waaronder competentie 3.0 (PL/I taalbasis) en competentie 4.0 (PL/I programmeerpraktijk).
Daarnaast wordt hier ook Competentie 1.0 (Fundamenten in programmeren) opgefrist in een mainframe context.
Studenten leren de specifieke syntaxis en declaraties m van PL1 samen met de programmastructuren.
Dit zorgt ervoor dat ze niet alleen code leren reproduceren maar ook begrijpen hoe PL1 logisch is opgebouwd binnen een professionele omgeving.
Deze module vormt de bouwstenen waarop verder kan gebouwd worden om te werken binnen complexe legacy systemen als een developer.


\subsubsection[short]{CICS Core Concepts}%
\label{sec:core-cics}

De module \textbf{CICS Core Concepts} is opgenomen omdat kennis van PL1 alleen niet volstaat wanneer men wil werken in transactionele mainframesystemen.
...
Het is dus belangrijk dat de student inzicht krijgt in de werking van CICS als systeemlaag naast het gebruik binnen PL1 zelf.
De module legt uit wat transacties, tasks en resources zijn zodat de student de noodzakelijke conceptuele basis heeft om later PL1 programma’s succesvol in een transactionele omgeving te laten draaien.
Hiermeer volbrengt deze module de competentie 5.0 (CICS concepten).
...
In deze module leren studenten het onderscheid maken tussen de vershillende soorten fouten en maken ze kennis met het gebruik van IBM message manuals.
Door deze eigenschappen kan de module gekoppeld worden aan competentie 10.0 (Foutanalyse en troubleshooting).
In technische opleidingen is debugging een fundamentele vaardigheid, het geeft studenten een beter beeld tijdens het programeren wanneer ze geconfronteerd worden met een probleem.
Doordat hier een apparte module is voor voorzien zijn studenten beter gewapend tijdens hun eerste weken op de werkvloer wanneer ze zelfstandig problemen moeten oplossen.
...
Nadat de student inzicht heeft verworven in zowel PL1 als CICS afzonderlijk, is het noodzakelijk om te tonen hoe beide in de praktijk met elkaar geïntegreerd worden.

Deze module integreert de competenties: 6.0 (CICS programmeerbasis), 7.0 (Gegevensuitwisseling in CICS), 8.0 (Build- en compileflow) en 11.0 (Codeanalyse en onderhoud) om tot een geheel te bekomen.
...
Deze module is daarom belangrijk als afsluiting van het leertraject, want studenten leren om de geïsoleerde leerinhouden toe te passen in een meer realistische mainframeomgevingomgeving. 

Samen zorgen deze zes modules voor een opbouw die logishe en opbouwend is. 
...
Hierdoor ontstaat er een leeromgeving die studenten aanzet om verder te gaan met de theoretisch kennis die word opgedaan.
