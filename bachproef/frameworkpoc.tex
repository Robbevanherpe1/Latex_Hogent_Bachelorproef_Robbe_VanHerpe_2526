...
Qua structuur leunt het competentieframework op gevestigde IBM frameworks, waaronder het IBM z/OS System Administrator Level 1 framework \autocite{ibm_z_sysadmin_level1}. 
...
Het gebruik van lussen, variabelen en datatypes zijn concepten die aan de basis liggen van elke programmeertaal.
Hierdoor is het heel waardevol om deze te beheersen voor het programmeren met PL/1 en CICS.

\subsection{Praktijk gericht}
\label{sec:Praktijk-gericht}

Een derde criterium is een sterke focus op de praktijk.
Het framework is bewust niet puur theoretisch opgebouwd.
Er is juist veel aandacht besteed voor de daadwerkelijke vaardigheden die op de werkvloer nodig zijn, zoals het efficiënt kunnen lezen en veilig aanpassen van bestaande legacy code.
...
Niveau 2 gaat dieper in op het geheugenbeheer.
Hier leert de ontwikkelaar werken met dynamic storage allocation via pointers en based of controlled variabelen. 
...
De leersite bevat meerdere schermen en componenten waaronder cursusoverzichten, hoofdstukken, theoriepagina’s, oefeningen en quizes waarbij de UI dynamisch mee evolueert met de selectie van de gebruiker.
...
\begin{itemize}
  ...
  \item Verschillende onderdelen waaronder: theorie,oefeningen en de quiz delen dezelfde navigatiestructuur en layout wat component hergebruik stimuleert.
\end{itemize}
...
In dit project is expliciet gekozen om dark mode te implementeren via de class strategie, aangezien deze aanpak uitstekend samenwerkt met Tailwind en eenvoudig te onderhouden is.
...
snelle ontwikkeling, lage kost, veilige contentweergave en een structuur die het toevoegen van nieuwe PL/1 en CICS leermodules zo eenvoudig mogelijk maakt.
...
\begin{figure}
   \centering
   \includegraphics[width=0.8\textwidth]{../graphics/mainscreen.png}
   \caption[Startscherm figuur.]{\label{fig:Startscherm}Website Startscherm.}
 \end{figure}

\subsection{Origins and Evolution of PL/I and CICS}%
...
\subsection{Environment Setup and ISPF Fundamentals}%
...
Legacy technologieën zoals ISPF hebben een steile leercurve voor nieuwe developers.
Daarom wordt er in dit thema ook gefocust op alternatieven zoals Zowe Explorer.
...
\subsection{PL/I and CICS Integration Mastery}%
...
\begin{figure}
   ...
   \caption[Theorie figuur.]{\label{fig:Theorie}Website theorie module.}
 \end{figure}
...
\subsection{Oefeningen in modules}%
...
\begin{figure}
   ...
   \caption[Oefeningen figuur.]{\label{fig:Oefeningen}Website Oefeningen module.}
 \end{figure}
...
\begin{figure}
   ...
   \caption[Quiz figuur.]{\label{fig:Quiz}Website quiz module.}
 \end{figure}
...
\section{Validatie van het framework en de PoC}%
\label{sec:validatie-framework-poc}

Om na te gaan of het opgestelde competentieframework en de bijhorende leersite bruikbaar zijn voor de doelgroep, werd binnen het tijdsbestek van deze bachelorproef een eerste validatie uitgevoerd.
Deze validatie verliep in twee stappen: een validatie door professionals uit het werkveld en een validatie door medestudenten van Hogeschool Gent.

\subsection{Validatie door professionals}%
\label{sec:validatie-professionals}

In een eerste stap werden het framework en de leersite voorgelegd aan de co-promotor van deze bachelorproef.
De co-promotor kreeg toegang tot het platform en kon de verschillende modules in eigen tempo doorlopen.
Achteraf volgde een mondelinge feedbacksessie waarin de inhoud en de bruikbaarheid van het platform werden besproken.

De feedback was overwegend positief.
De co-promotor gaf aan dat de algemene opbouw van de leersite logisch in elkaar zit en dat de verschillende thema's op een doordachte manier zijn opgebouwd.
Ook de gebruiksvriendelijkheid van het platform werd als sterk punt benoemd: de navigatie tussen modules verloopt vlot en de drempel om met de inhoud aan de slag te gaan is laag genoeg voor een starter.
Daarnaast werd benadrukt dat het een sterke keuze is om eerst de historische en conceptuele context te behandelen, voordat PL/I en CICS zelf aan bod komen.
Voor startende developers helpt dit om de leerstof beter te plaatsen.

\subsection{Validatie door medestudenten}%
\label{sec:validatie-studenten}

In een tweede stap werd de leersite getest door enkele medestudenten van Hogeschool Gent.
Drie studenten kregen tijdens een korte demo eerst een rondleiding doorheen het platform en de verschillende modules.
Vervolgens konden ze zelf met de leersite aan de slag en de inhoud in eigen tempo verkennen.
Tot slot werd hun ervaring besproken tijdens een mondelinge feedbacksessie.

De feedback van de studenten richtte zich vooral op de gebruiksvriendelijkheid en de leesbaarheid van de inhoud.
Ze gaven aan dat de vaste opbouw met theorie, oefeningen en quiz duidelijk werkt, en dat de oefeningen hen hielpen om de theorie beter vast te zetten.
Daarnaast signaleerden ze ook enkele technische werkpunten. Zo werkte het scrollen op de leersite niet correct in alle browsers, en gaven de studenten aan dat ze graag een mobiele versie van het platform zouden gebruiken.
Op basis van deze feedback werden de scrollproblemen verholpen zodat de leersite vlot werkt in alle moderne browsers, en werd de layout aangepast zodat het platform ook bruikbaar is op mobiele toestellen.

\subsection{Beperkingen van de validatie}%
\label{sec:validatie-beperkingen}

De uitgevoerde validatie geeft een eerste, positief beeld van de bruikbaarheid van zowel het framework als de leersite. 
Toch moet er rekening worden gehouden met enkele duidelijke beperkingen.
Het aantal personen dat aan de validatie deelnam is beperkt slechts één professional en drie studenten.
Bovendien was de testperiode beperkt in tijd, waardoor geen uitspraken kunnen worden gedaan over de langetermijn effecten op het leerproces of op de inzetbaarheid in een professionele omgeving.
Deze beperkingen worden verder besproken in Sectie~\ref{sec:beperkingen-reflectie} van de conclusie, samen met concrete aanbevelingen om de validatie in toekomstig onderzoek uit te breiden.
